...
class PlayState extends MusicBeatState
{
	...
	var resistancebarHelper:Int;
	public static var resistanceNote:Int;
	...
	private var resistanceBG:AttachedSprite;
	...
	var dotGroup1:FlxSprite;
	var dotGroup2:FlxSprite;
	var dotGroup3:FlxSprite;
	var dotGroup4:FlxSprite;
	...
	override public function create()
	{
		...
		camHUD.visible = !ClientPrefs.hideUI;
		...
		if (ClientPrefs.staticcamera == true)
				defaultCamZoom -= 0.1;
				FlxG.camera.scroll.y += -30;
		isPixelStage = stageData.isPixelStage;
		if (curStage == 'void' || curStage == 'glitchyvoid')
			{
				BF_X = 920;
				BF_Y = 250;
				GF_X = 425;
	...
	healthBarBG.sprTracker = healthBar;
		...
		if (ClientPrefs.staticcamera == false)
			{
	...
	function startSong():Void
	{
		...
		FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song, CoolUtil.difficulties[storyDifficulty]), 1, false);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
			{
				vocals = new FlxSound().loadEmbedded(Paths.voices(PlayState.SONG.song, CoolUtil.difficulties[storyDifficulty], SONG.player1));
			}
		else
			{
				vocals = new FlxSound();
	...
	override public function update(elapsed:Float)
	{
		...
		FlxG.watch.addQuick('diff', CoolUtil.difficulties[storyDifficulty]);
		FlxG.watch.addQuick('erectLocation', Paths.voices(PlayState.SONG.song, CoolUtil.difficulties[storyDifficulty], SONG.player1));
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote && !note.resistancething)
			{
				combo += 1;
				popUpScore(note);
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					case 'week1_nomiss' | 'week2_nomiss' | 'week3_nomiss' | 'week4_nomiss' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'CLASSIC' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
